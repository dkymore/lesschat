definePlugin(
  '@plugins/com.msgbyte.openapi/useOpenAppAction-74be9afd.js',
  ['exports', '@capital/common', './index-c235bf17'],
  function (exports, common, index) {
    'use strict';

    function useOpenAppAction() {
      const { refresh, appId, capability, onSelectApp } =
        index.useOpenAppInfo();
      const [{ loading }, handleChangeAppCapability] = common.useAsyncRequest(
        async (targetCapability, checked) => {
          const newCapability = [...capability];
          const findIndex = newCapability.findIndex(
            (c) => c === targetCapability
          );
          if (checked) {
            if (findIndex === -1) {
      ...
      const [, handleSetAppInfo] = common.useAsyncRequest(
      ...
      const [, handleUpdateOAuthInfo] = common.useAsyncRequest(
        async (name, value) => {
      ...
      const [, handleUpdateBotInfo] = common.useAsyncRequest(
        async (name, value) => {
