  ...
  function (common) {
      ...
      (function () {
        ...
        var removeFaviconTag = function () {
          ...
          for (var i = 0, len = links.length; i < len; i++) {
            ...
            if (
              exists &&
              (links[i].getAttribute('rel') || '').match(/\bicon\b/i)
            ) {
        ...
        var abbreviateNumber = function (label) {
          ...
          for (var i = 0; i < metricPrefixes.length; ++i) {
            if (label >= metricPrefixes[i][1]) {
              label =
                round(label / metricPrefixes[i][1]) + metricPrefixes[i][0];
              break;
            }
          }
        ...
        Tinycon.setOptions = function (custom) {
          ...
          for (var key in defaults) {
            options[key] = custom.hasOwnProperty(key)
              ? custom[key]
              : defaults[key];
          }
          return this;
    ...
    function initNotify() {
      ...
      common.sharedEvent.on('receiveUnmutedMessage', (message) => {
        var _a, _b;
        const currentUserId =
          (_a = common.getGlobalState()) == null ? void 0 : _a.user.info._id;
        ...
        if (hidden || isBlur) {
          if (Notification.permission === 'granted') {
            Promise.all([
              ...
              message.groupId
                ? common
                    .getCachedBaseGroupInfo(message.groupId)
                    .then((d) => d.name)
                : Promise.resolve(Translate.dm),
            ]).then(([userInfo, scopeName]) => {
              var _a2, _b2;
              const nickname =
                (_a2 = userInfo == null ? void 0 : userInfo.nickname) != null
                  ? _a2
                  : '';
              const icon =
                (_b2 = userInfo == null ? void 0 : userInfo.avatar) != null
                  ? _b2
                  : void 0;
