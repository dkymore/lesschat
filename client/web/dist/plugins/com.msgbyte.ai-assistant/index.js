definePlugin(
  '@plugins/com.msgbyte.ai-assistant',
  [
    '@capital/common',
    '@capital/component',
    'react',
  ...
  function (common, component, React, axios, styled) {
    'use strict';
...
`;
    const AssistantPopover = React__default['default'].memo((props) => {
      ...
      const [{ loading, value }, handleCallAI] = common.useAsyncRequest(
        async (question) => {
          const { data } = await axios__default['default'].post(
            'https://yyejoq.laf.dev/chatgpt',
            {
              question,
            }
          );
                ...
                await Promise.all(
                  [...messages]
                    ...
                    .map(async (item) => {
                      var _a;
                      return `${
                        (await common.getCachedUserInfo(item.author)).nickname
                      }: ${common
                        .getMessageTextDecorators()
                        .serialize((_a = item.content) != null ? _a : '')}`;
                    })
                )
              ).join('\n');
